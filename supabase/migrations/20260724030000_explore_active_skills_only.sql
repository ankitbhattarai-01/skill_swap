-- Hide retired skills from Explore.
--
-- explore_teachers / explore_learners join public.skills with no activation
-- filter, so a profile that still carries a skill retired by
-- 20260724000000_curated_skill_catalog keeps appearing on the discovery cards.
-- Clicking through would offer to book a skill that can no longer be selected.
--
-- The companion prune migration (20260724010000) removes those profile rows and
-- makes the problem disappear at the source, but this filter is what keeps it
-- from coming back the next time an admin retires a skill without pruning.
--
-- Verbatim copies of the 20260612220000 definitions with one added predicate
-- (`AND s.is_active`) in each. Signatures and result columns are unchanged, so
-- CREATE OR REPLACE is sufficient and the existing grants carry over.
